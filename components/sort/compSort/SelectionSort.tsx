...
export function SelectionSort(): JSX.Element {
    ...
    async function selectionSort(arr: number[], setArr: Dispatch<SetStateAction<number[]>>) {
    ...
    const handleSort = async () => {
        setSortStart(true);
        await selectionSort(orders, setOrders);
        setSortStart(false);
        setCurrent(null);
        setCurrent2(null);
    };
    ...
    return (
        <div className="px-5 max-w-3xl w-full first:mt-20">
            <p className="text-2xl md:text-3xl">Selection Short</p>
