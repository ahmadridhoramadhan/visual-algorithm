...
export function CocktailShakerSort(): JSX.Element {
    ...
    const cocktailShakerSort = async (arr: number[], setArr: Dispatch<SetStateAction<number[]>>) => {
        let left = 0;
    ...
    const handleSort = async () => {
        setSortStart(true);
        await cocktailShakerSort(orders, setOrders);
        setSortStart(false);
        setCurrent(null);
        setCurrent2(null);
    };
    return (
        <div className="px-5 max-w-3xl w-full first:mt-20">
            <p className="text-2xl md:text-3xl">Cocktail Shaker Sort</p>
